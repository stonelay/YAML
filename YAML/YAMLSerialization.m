...
__YAMLSerializationObjectWithYAMLDocument (yaml_document_t *document, YAMLReadOptions opt, NSError **error) {
    ...
    for (node = document->nodes.start, i = 0; node < document->nodes.top; node++, i++) {
        switch (node->type) {
            ...
            NSString *valueString = [[NSString alloc] initWithUTF8String: (const char *)node->data.scalar.value];

            id value = nil;

            if (!(opt & kYAMLReadOptionStringScalars)) {
                if (strcmp((const char *)node->tag, YAML_BOOL_TAG) == 0) {
                    NSString *v = [valueString lowercaseString];
                    // true values: yes, true, y, on

                    if ([v isEqualToString:@"yes"] || [v isEqualToString:@"true"] || [v isEqualToString:@"y"] || [v isEqualToString:@"on"]) {
                        value = [@YES retain];
                    } else {
                        value = [@NO retain];
                    }
                    ...
                    value = [@([valueString floatValue]) retain];

                } else if (strcmp((const char *)node->tag, YAML_INT_TAG) == 0) {
                    if ([[valueString lowercaseString] hasPrefix:@"0x"]) {  // hex
                        NSString *hex = [valueString substringFromIndex:2];
                        value = [@(strtoull([hex UTF8String], NULL, 16)) retain];
                    } else {
                        value = [@([valueString integerValue]) retain];
                    }
                }
            }
            ...
            if (value == nil) {
                value = [[stringClass alloc] initWithUTF8String: (const char *)node->data.scalar.value];
            }
        ...
        switch (node->type) {
            ...
            case YAML_MAPPING_NODE:
                for (pair = node->data.mapping.pairs.start; pair < node->data.mapping.pairs.top; pair++) {
                    ...
                    NSString *key = objects[pair->key - 1];
                    id value = objects[pair->value - 1];

                    if ([key isKindOfClass:[NSString class]] && [key isEqualToString:@"<<"]) {

                        if ([value isKindOfClass:[NSArray class]] && [value[0] isKindOfClass:[NSDictionary class]]) {
                            for (NSDictionary *dict in value) {
                                [objects[i] addEntriesFromDictionary:dict];
                            }
                        } else if ([value isKindOfClass:[NSDictionary class]]) {
                            [objects[i] addEntriesFromDictionary:value];
                        } else {
                            [objects[i] setObject: value
                                           forKey: key];
                        }
